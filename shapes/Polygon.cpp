...
Polygon::Polygon(Vector2f &position, float &angle, int numVertex, Vector2f data[]) :
...
Vector2f Polygon::getVertexTransformed(int index){
	return Vector2f(FromAngle(1,angle) * vertex[index].x + FromAngle(1, angle - PI/2) * vertex[index].y) + Position;
}
...
bool Polygon::Collide(Polygon A, Polygon B) {
	...
	for (int i = 0; i < A.numVertexs; i++) {
		Vector2f E = A.getVertexTransformed((i+1)%A.numVertexs) - A.getVertexTransformed(i);
	...
	for (int i = 0; i < B.numVertexs; i++) {
		Vector2f E = B.getVertexTransformed((i+1)%B.numVertexs) - B.getVertexTransformed(i);
		Vector2f N = Vector2f(-E.y, E.x);
...
void Polygon::CalculateInterval(Vector2f Axis, Polygon P, float& min, float& max) {
