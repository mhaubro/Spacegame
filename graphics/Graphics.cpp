...
void Cam::Vertex2f(float x, float y) {
	...
	while (X > (WORLD_SIZE * CHUNK_SIZE)/2) {
		X -= WORLD_SIZE * CHUNK_SIZE;
	}
	while (X < -(WORLD_SIZE * CHUNK_SIZE)/2) {
		X += WORLD_SIZE * CHUNK_SIZE;
	}

	if (X < SCREEN_MIN_X || X > SCREEN_MAX_X || Y < SCREEN_MIN_Y
			|| Y > SCREEN_MAX_Y) {
		return;
	}
...
unsigned int blend(unsigned int color1, unsigned int color2, float f) {
	unsigned int b = (unsigned int) ((color1 & 0xff) * f)
			+ ((color2 & 0xff) * (1 - f));
	unsigned int g = (unsigned int) ((color1 & 0xff00) * f)
			+ ((color2 & 0xff00) * (1 - f));
	unsigned int r = (unsigned int) ((color1 & 0xff0000) * f)
			+ ((color2 & 0xff0000) * (1 - f));
